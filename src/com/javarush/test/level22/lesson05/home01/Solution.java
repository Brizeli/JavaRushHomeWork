...
public class Solution {
    ...
    public String getPartOfString(String string, String threadName) {
        ...
        try {
            ...
            res=string.substring(ind1,ind2).trim();
        } catch (Exception e) {
            if (threadName.equals(FIRST_THREAD_NAME))throw new RuntimeException(e.toString(),new TooShortStringFirstThreadException());
            else if (threadName.equals(SECOND_THREAD_NAME)) throw new RuntimeException(e.toString(),new TooShortStringSecondThreadException());
