...
public class Solution {
    ...
    public <T extends Item> T getProxy(Class<T> itemClass, Class<?>... dopItemClasses) {
        ArrayList<Class<?>> interfaces=new ArrayList<>();
        interfaces.add(itemClass);
        for (Class<?> dopItemClass : dopItemClasses)
            interfaces.add(dopItemClass);
        return (T) Proxy.newProxyInstance(itemClass.getClassLoader(),interfaces.toArray(new Class<?>[interfaces.size()]), new ItemInvocationHandler());
    }
